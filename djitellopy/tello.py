...
class Tello:
    ...
    RESPONSE_TIMEOUT = 100  # in seconds
    TAKEOFF_TIMEOUT = 60  # in seconds
    ...
    def __init__(self,
        ...
        global threads_initialized, client_socket, drones, STATE_LOG
        ...
        if not threads_initialized:
            # Run Tello command responses UDP receiver on background - thread for command
            client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            client_socket.bind(('', Tello.CONTROL_UDP_PORT))#bind the ports
            response_receiver_thread = Thread(target=Tello.udp_response_receiver)#create background thread
            response_receiver_thread.daemon = True
            response_receiver_thread.start()

            # Run state UDP receiver on background - thread for state
    ...
    def udp_state_receiver():
        ...
        while True:
            try:
                ...
                tmp = Tello.parse_state(data)
                tmp['timestamp']= datetime.now().time()
                tmp['address']=address
                STATE_LOG.append(tmp)
            except Exception as e:
                Tello.LOGGER.error(e)
                break
